...
class ColoredChestKukaEnv(gym.Env):
    ...
    def _compute_reward_and_success(self, action: np.ndarray):
        """
        ...
        if self.reward_type == "advanced":
            ...
            if distance < self.success_distance:
                reward += 8.0 + 3.0 * next_close_steps
                reward -= 0.25 * action_norm
            else:
                reward -= 0.02 * action_norm

        self.consecutive_close_steps = next_close_steps
